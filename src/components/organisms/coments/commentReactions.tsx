...
const CommentReactions = ({ comments, commentData, reactions = [], globalUserIp = "" }: IComentReaction) => {
    const [comment, setComments] = useState(comments)
    const [isLike, setLike] = useState(false)
    const [isDislike, setDislike] = useState(false)
    const [likeReaction, setLikeReaction] = useState(reactions.filter(element => element.action === "like"));
    const [disLikeReaction, setDisLikeReaction] = useState(reactions.filter(element => element.action === "dislike"));
    const [likeCount, setLikeCount] = useState(likeReaction.length + +comment.attributes.admin_like);
    ...
    useEffect(() => {
        const myIp = reactions.filter((element) => {
            if (element.user_email === getUserEmailForIf && element.ip_address == globalUserIp) {
                return element
            }
        })
    ...
    const updateCommentReaction = async (commentData: IComentData, action: string) => {
        try {
    ...
    const onClick = async (e: any) => {
        ...
        if (buttonType === 'like') {
            ...
            setLike(false);
            setDislikeCount((dislikeCount) => isDislike ? dislikeCount - 1 : dislikeCount + 1);
            if (isLike) {
    ...
    return (
        <>
            <div className="comment-footer_menu_reaction_block_button" itemProp="interactionStatistic" itemType="https://schema.org/InteractionCounter" itemScope>
                <button itemProp="interactionType" content="https://schema.org/LikeAction" name="like button" data-type="like" onClick={(e) => onClick(e)} className={isLike ? "comment-footer_menu_reaction_button active" : "comment-footer_menu_reaction_button"}>
                    {/* <picture>
                        <Image 
            ...
            <div itemProp="interactionStatistic" itemType="https://schema.org/InteractionCounter" itemScope className="comment-footer_menu_reaction_block_button">
                <button itemProp="interactionType" content="https://schema.org/DislikeAction" name="dislike button" data-type="dislike" onClick={(e) => onClick(e)} className={isDislike ? "comment-footer_menu_reaction_button active" : "comment-footer_menu_reaction_button"} >
                    {/* <picture>
