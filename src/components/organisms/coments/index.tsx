      ...
      <section>
        <div className="comments-tree">
          <header className="comments-header">
            <h4>
              {' '}
              {comments.length} {$t[locale].comment.comments}
            </h4>
          </header>
          <section>
            <h3 className="notShowOnPage">{$t[locale].comment.placeholder}</h3>
            <TextArea saveDraft={saveDraftComment} sendMessage={sendMessage} />
          </section>
        </div>
        <aside>
          <ModalConfirm
      ...
      <section itemScope itemType="https://schema.org/DiscussionForumPosting">
          ...
          return (
            <Image
              ...
              src={`${sanitizeImageUrl(
                NEXT_STRAPI_BASED_URL + image?.attributes.url
              )}`}
              alt={image?.attributes?.alternativeText || 'alt text'}
              key={image.id}
            />
          );
        })}
        <h3 itemProp="headline" className="notShowOnPage">
          {$t[locale].comment.comments}
        </h3>
        ...
        {comments.map((comment, index) => {
          ...
          const findFatherName = () => {
            ...
            return (
              <Link
                href={`/user/${name?.attributes?.user?.data?.attributes?.username}`}
              >
                {name?.attributes?.user?.data?.attributes?.real_user_name}
              </Link>
            );
          };
          ...
          return (
            <article
              className={
                showedComent >= index + 1
                  ? `${getCommentClass(father.data)} block`
                  : `${getCommentClass(father.data)} none`
              }
                ...
                <div className="post-body">
                  <header className="comment__header">
                    ...
                    >
                      <link
                        itemProp="url"
                        href={`${NEXT_FRONT_URL}/user/${user.data.attributes.username}`}
                      />
                    ...
                    <span className="post-meta">
                      <time
                        ...
                        dateTime={admin_date}
                        title={formatDateTime(admin_date, true)}
                      >
                  ...
                  <footer className="comment__footer">
                    <nav className="comment-footer__menu">
                      ...
                      >
                        <span className="text reply-button">
                          {$t[locale].comment.reply}
                        </span>
                      </button>
                    </nav>
                  </footer>
        ...
        <button
          ...
          className={
            commentCoutn - showedComent <= 0 ? 'none' : 'show_more_button'
          }
        ...
        >
          <span
            className={
              isButtonLoading
                ? 'loading show_more_button_span '
                : 'show_more_button_span'
            }
          >
            <picture>
              <Image
          ...
          {replaceText(
            `${$t[locale].blog.showMore} ${commentCoutn - showedComent}`
          )}
        </button>
      </section>
    </>
