...
export default async function getRandomPopularNews(locale: string, count= 4, collectionType = "blogs", isPopular = true) {
  const getPage = await server.get(`/${collectionType}?${fieldsQuery}&${populateQuery}&locale=${locale === 'ua' ? 'uk' : locale}&pagination[page]=1&pagination[pageSize]=50${isPopular?"&filters[is_popular][$eq]=true":""} `);
