...
export default function Home({
  ...
  const activeHeading = useMemo(
    () =>
      headings.find(
        element =>
          element?.attributes?.Name === query?.heading &&
  ...
  return (
    <>
      <Head>
        <title>
          {activeHeading?.attributes?.blog_title
            ? activeHeading?.attributes?.blog_title
            : seoData?.Title
            ? seoData?.Title
            : $t[locale].news.title}
        ...
        <meta
          name="description"
          content={
            activeHeading?.attributes?.blog_descriptions
              ? activeHeading?.attributes?.blog_descriptions
              : seoData?.Description
              ? seoData?.Description
              : $t[locale].news.description
          }
        />
        <meta
          name="keywords"
          content={
            activeHeading?.attributes?.keywords
              ? activeHeading?.attributes?.keywords
              : seoData?.Keywords
              ? seoData?.Keywords
              : $t[locale].news.keywords
          }
        />
                        ...
                        <h1 className="text-white animated d-flex align-items-center flex-wrap slideInLeft">
                          <nav>
                            <ul
                              ...
                              className="text-white animated d-flex align-items-center flex-wrap slideInLeft"
                              style={{
                                paddingLeft: 0,
                                listStyleType: 'none',
                                display: 'flex',
                            ...
                            >
                              ...
                              >
                                <Link
                                  href={`/news`}
                                  itemProp="item"
                                  className="d-inline text-white heading_title"
                                >
                                  <span itemProp="name">
                                    {$t[locale].news.all}
                                  </span>
                              ...
                              {headings.map((heading, index) => {
                                ...
                                return (
                                  ...
                                  >
                                    <Link
                                      href={`/news?heading=${headingName}`}
                                      itemProp="item"
                                    >
                                      <h2
                                        className="d-inline heading_title text-white heading_name"
                                        itemProp="name"
                                      >
                  ...
                  <div className="col article-col pe-md-2">
                    ...
                    >
                      <section
                        ...
                        className=" col article-col gap-5  d-flex flex-column col  "
                      >
                        {pages.map((page, index) => {
                          ...
                          const imageUrl = page?.attributes?.image?.data
                          ...
                          return (
                            ...
                            >
                              ...
                              <meta
                                ...
                                content={`UserComments:${comments.data.length}`}
                              />
                              {imageUrl && (
                                    ...
                                    >
                                      <Image
                                        ...
                                        src={`${sanitizeImageUrl(
                                          NEXT_STRAPI_BASED_URL + imageUrl
                                        )}`}
                                        width={260}
                                        height={100}
                                        alt="img"
                              ...
                              <div
                                className="col-sm-9 col-txt d-flex flex-column justify-content-between blog_text_block  "
                                style={{ paddingRight: 0, width: '100%' }}
                              >
                                <h2 className="entry-title text-uppercase">
                                  <Link
                                    itemProp="mainEntityOfPage"
                                    className="entry-title text-uppercase h4"
                                    href={url}
                                  >
                                    {hyphenateText(page_title)}
                                  </Link>
                                </h2>
                                    ...
                                    <div className="align-items-center d-flex gap-3">
                                      ...
                                      >
                                        <Link
                                          ...
                                          href={`/news?heading=${heading.data?.attributes.Name}`}
                                          className="text-info text-capitalize fw-bold a-not-img"
                                        >
...
export async function getServerSideProps({ query, locale }) {
  ...
  const [
    ...
    server.get('/social'),
    server
      .get(`/headings-news?locale=${Locale}`)
      .catch(() => ({ data: { data: [] } })), // Обробка помилок
    server.get(
      `/newss?locale=${Locale}&pagination[page]=${page}&pagination[pageSize]=${perPage}${filter}&sort[0]=admin_date:desc&populate[article][populate][author]=*&populate[article][populate][images]=*&populate[comments]=user_name,CustomHistory&populate=image&populate[faq]=*&populate[rating]=*&populate[heading]=*&populate[code]=*&populate[howto]=*`
    ),
